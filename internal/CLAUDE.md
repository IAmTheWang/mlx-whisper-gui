...
- `config` — lowest layer. Reads/writes `~/Library/Application Support/whisper-gui/config.json` (user-overridden binary paths, whisper.cpp model directory, optional VAD model path, default engine). No dependencies on other `internal/` packages.
