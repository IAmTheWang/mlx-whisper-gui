...
- **`condition_on_previous_text` is explicitly set to `False`** in `runJob`'s mlx_whisper invocation. mlx_whisper defaults this to `True` (feed each window's transcript back in as the next window's prompt), which helps cross-window consistency but means a single bad window — most commonly a silent/no-speech stretch that gets mistranscribed into a plausible-sounding phrase — gets fed back into the model and can lock it into repeating that exact line verbatim for the rest of the file. A repo-wide scan found this had silently corrupted a large fraction of every job's output since the app's very first use, long before it was noticed. See `notes/mlx-whisper-repetition-loop-bugfix.md` for the investigation and before/after data. Don't remove this flag without re-reading that writeup.
