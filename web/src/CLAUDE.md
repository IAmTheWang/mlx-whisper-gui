...
- `api.ts` — typed `fetch` wrappers for every backend endpoint (`/api/browse`, `/api/models` (takes an `EngineID`, defaults to `'mlx'`), `/api/settings`, `/api/jobs*`). `request<T>()` centralizes error handling: a non-OK response throws using the server's JSON `{error}` body when present, falling back to the HTTP status text.
...
- `types.ts` — all shared TS interfaces (`Job`, `BrowseEntry`, `ModelInfo`, `LogEvent`, `EngineID`, etc.), hand-kept in sync with the Go JSON structs in `internal/server`/`internal/jobmanager`. There is no codegen — when a backend field changes, update this file too.
- `main.ts` — top-level composition: lays out the header (Settings button) and the four panes (`browser`, `form`, `list`, `detail`), wires selection → job form → job creation → job list refresh → job detail. This is the only place that owns cross-view state (`selectedPaths`, `currentEngine`). On startup it fetches `getSettings()` once to pick the initial engine: an explicit `defaultEngine` wins, otherwise it auto-picks whichever engine's binary actually resolves (so a fresh Mac with only whisper.cpp installed doesn't land on an unusable `mlx` default). Before creating jobs, it also checks `browser.getEntry(path).hasSrt` for each selected file and shows a native `confirm()` listing any that already have a subtitle, since job creation always overwrites.
